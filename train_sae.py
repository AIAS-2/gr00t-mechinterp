...
import argparse
from sparse_autoencoder import SparseAutoencoder, TokenActivationDataset, train_sae
...
if __name__=="__main__":
    ...
    layer_idx   = 11                            # the "12-th layer (0-indexed)"
    image_dir   = "openimages_subset/images_train"
    paths       = [os.path.join(image_dir, f) for f in os.listdir(image_dir)]
    device      = torch.device("cuda")

    # 1) Get the frozen vision encoder --------------------------------------------
    vit, txt, tok = load_models(device)

    # 2) Build a dataset & dataloader that streams patch tokens -------------------
    ...
    # 3) Instantiate and train the SAE --------------------------------------------
    sae  = SparseAutoencoder(input_dim=1152, hidden_dim=2048, sparsity_weight=1e-3)
    train_sae(sae, dl, n_epochs=5, lr=1e-3, device=device)
    
    ckpt_path = f"2048_checkpoints/sae_layer{layer_idx + 1}.pt"
    torch.save(sae.state_dict(), ckpt_path)
    print(f"[✓] SAE saved to {ckpt_path}")



    # 4) Inspect which SAE units respond to which kinds of patches ----------------
    #from sparse_autoencoder import top_k_patches
    #top_dict = top_k_patches(sae, dl, k=30, device=device)

    # Example: print the mean token vector of the most active patches for unit-7
    #unit = 7
    #mean_vec = torch.stack(top_dict[unit]).mean(0)
    #print(f"Unit {unit} average activation vector ≈ {mean_vec[:10]}…")
